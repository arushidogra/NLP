...
STOP WORDS ARE REMOVED FROM FEATURE WORDS
...
def main():
	...
	result2=[]
	punctuations='''!()-[]{};:'"\|,<>./?@#$%^&*_~'''
	f=open('hindi_stopwords.txt','r')
	stop_word=f.read()
	stop_word=re.split(' |\n',stop_word)
	stop_word=[word for word in stop_word if word!='']
	#print result1
	tokens=[]
	for i in xrange(0,len(result1)):
		if result1[i][1].lower() not in punctuations and result1[i][1].lower() not in stop_word:
			result2.append(result1[i][1])
	for i in xrange(0,len(final_tokens)):
		if final_tokens[i].lower() not in punctuations:
...
#	print len(centers)
	...
	os.system("touch stopwords_removed.txt")
	a = open("stopwords_removed.txt","r+")
	for i in group2:
		a.write(str(i+1) + ':')
